...
\begin{frame}{K-Means: La Idea}
  \vspace{-4pt}
  {\footnotesize\color{arcaDark}\bfseries
    K-Means agrupa $N$ observaciones en $K$ clusters.
    Cada cluster tiene un \textbf{centroide} (punto central).
    Cada observaci\'on pertenece al cluster cuyo centroide est\'a m\'as cerca.}

  \vspace{6pt}
  {\footnotesize
  \begin{enumerate}\setlength{\itemsep}{4pt}
    \item[{\color{arcaRed}\bfseries 1.}]
      Elegir $K$ (n\'umero de clusters). Lo decidimos nosotros.
    \item[{\color{arcaRed}\bfseries 2.}]
      Colocar $K$ centroides en posiciones \textbf{aleatorias}.
    \item[{\color{arcaRed}\bfseries 3.}]
      \textbf{Asignar} cada punto al centroide m\'as cercano.
    \item[{\color{arcaRed}\bfseries 4.}]
      \textbf{Recalcular} cada centroide como el promedio de sus puntos.
    \item[{\color{arcaRed}\bfseries 5.}]
      Repetir pasos 3--4 hasta que los centroides \textbf{no se muevan}.
  \end{enumerate}
  }

  \vspace{4pt}
  {\scriptsize\color{textMuted}
    \faLightbulb\hspace{4pt} Es un algoritmo \textbf{iterativo}:
    asignar $\to$ recalcular $\to$ asignar $\to$ recalcular\ldots
    hasta converger. T\'ipicamente converge en 10--20 iteraciones.}
\end{frame}
...
\begin{frame}[shrink=5]{Ejercicio 2: Segmentaci\'on desde Cero}
  \vspace{-4pt}
  {\footnotesize\color{arcaDark}\bfseries
    \faClock\hspace{4pt} 15 minutos --- en el notebook:}
  ...
  {\footnotesize
  \begin{enumerate}\setlength{\itemsep}{4pt}
    \item Carguen el dataset Mall Customers.
    \item Hagan el m\'etodo del codo ($K = 1$ a 10) con \texttt{px.line}.
    \item Elijan $K$ y corran K-Means.
    \item Calculen el perfil promedio con \texttt{groupby}.
    \item D\'enle un \textbf{nombre de negocio} a cada segmento
      (ej: ``Premium'', ``Ahorradores'', ``Oportunidad'').
    \item Scatter con Plotly coloreado por segmento.
    \item ?`Qu\'e campa\~na de marketing har\'ian para cada segmento?
  \end{enumerate}
  }
\end{frame}
