...
\begin{frame}{El Dataset: D\'igitos Escritos a Mano}
  ...
  {\footnotesize\color{arcaDark}\bfseries
    ...
    escritos a mano, cada una de $8 \times 8$ pixeles.}
...
\begin{frame}[shrink=18]{?`Por Qu\'e No Es Lineal?}
  \vspace{-4pt}
  \begin{center}
    \includegraphics[width=0.88\textwidth]{fig_digits_nolineal.png}
  \end{center}

  \vspace{-2pt}
  {\scriptsize\color{textMuted}
    \faLightbulb\hspace{4pt} \textbf{Izquierda:} dos pixeles cualesquiera
    no alcanzan para separar d\'igitos --- se solapan completamente.
    No hay ninguna l\'inea recta que los separe.
    \textbf{Derecha:} la matriz de correlaci\'on entre los 64 pixeles
    muestra una estructura compleja, con bloques y patrones ---
    no es una simple correlaci\'on lineal.
    Por eso \textbf{PCA (lineal) pierde estructura} y
    \textbf{t-SNE (no lineal) la captura}.}
\end{frame}
